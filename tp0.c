...
void send_hello(int socket, Alumno alumno) {
	...
	/*
	 * Si te estas preguntando porqué funciona el send y no es necesario “serializar”
	 * nada de la estructura es porque usa el atributo "__attribute__((packed))".
	 * Esto solamente funciona siempre y cuando la estructura no tenga punteros y
	 * los datos se envíen a procesadores con arquitecturas compatibles.
	 * En caso que alguna de las condiciones no se cumplan, es necesario crear un buffer
	 *  y serializar los datos antes de poder enviarlos.
	 *
	 * */
	if (resultado <= 0) {
	 ...
	 */
	close(socket);
	log_info(logger, "Hemos enviado la informacion del alumno!");
}
...
void * wait_content(int socket) {
	...
	if (recv(socket, header, sizeof(ContentHeader), 0) <= 0) {
		_exit_with_error(socket,
				"No se pudo recibir el encabezado del contenido", header);
	}
	...
	/*
	 * Una buena práctica sería chequear primero que el tamaño a reservar no sea demasiado grande.
	 * En caso que haya algún problema un mensaje que se recibió de forma inválida podría hacer que
	 * se reserve mucha memoria y haga fallar el programa.
	 * */
...
void send_md5(int socket, void * content) {
	...
	/*
	 ...
	 A diferencia de recibirlo, para mandarlo es mejor enviarlo todos de una,
	...
	/*
	 18.   Con todos listo, solo nos falta enviar el paquete que armamos y liberar la memoria que usamos.
